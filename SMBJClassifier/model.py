...
def prepare_graph_data(node_features, edge_indices, labels):
    """ Convert node features, edge indices, and labels to PyTorch Geometric Data objects. """
    data_list = []
    
    for i in range(len(node_features)):
        print(f"Processing graph {i}")
        
        # Check if node_features and edge_indices are lists of arrays
        print(f"Type of node_features[{i}]: {type(node_features[i])}")
        print(f"Shape of node_features[{i}]: {getattr(node_features[i], 'shape', 'N/A')}")
        
        print(f"Type of edge_indices[{i}]: {type(edge_indices[i])}")
        print(f"Shape of edge_indices[{i}]: {getattr(edge_indices[i], 'shape', 'N/A')}")

        # Ensure node_features and edge_indices are numpy arrays and reshape if necessary
        if isinstance(node_features[i], np.ndarray) and node_features[i].ndim == 1:
            node_features[i] = node_features[i].reshape(-1, 1)
        
        if isinstance(edge_indices[i], np.ndarray) and edge_indices[i].ndim == 1:
            edge_indices[i] = edge_indices[i].reshape(2, -1)  # Ensure it's in shape [2, num_edges]

        # Convert node features to PyTorch tensor
        x = torch.tensor(node_features[i], dtype=torch.float)
        
        # Convert edge indices to PyTorch tensor and ensure it's in the correct format
        edge_index = torch.tensor(edge_indices[i], dtype=torch.long).t().contiguous()  # Ensure it's a 2D tensor
...
def gnn_pretrain(num_classes, Train_Data, Train_Label, Test_Data, Test_Label):
    """ Trains a GNN model and extracts features """
    
    # Ensure that Train_Data and Test_Data are lists of PyTorch Geometric Data objects
    if not all(isinstance(data, Data) for data in Train_Data):
        raise ValueError("Train_Data should be a list of PyTorch Geometric Data objects.")
    if not all(isinstance(data, Data) for data in Test_Data):
        raise ValueError("Test_Data should be a list of PyTorch Geometric Data objects.")
    
    print(f"Type of Train_Data: {type(Train_Data)}")
    print(f"Type of Train_Data[0]: {type(Train_Data[0])}")
    print(f"Shape of Train_Data[0].x: {Train_Data[0].x.shape}")
    
    print(f"Type of Test_Data: {type(Test_Data)}")
    print(f"Type of Test_Data[0]: {type(Test_Data[0])}")
    print(f"Shape of Test_Data[0].x: {Test_Data[0].x.shape}")

    # Extract the number of node features
    num_node_features = Train_Data[0].x.shape[1]  # Assuming feature dimension is the second dimension

    # Create GNN model
    model = GNN(num_node_features=num_node_features, num_classes=num_classes)
    ...
    model.eval()
    train_features = [model(data).detach().numpy() for data in Train_Data]
    test_features = [model(data).detach().numpy() for data in Test_Data]
...
def runClassifier(approach, data, RR, group, num_group, sampleNum):
    """ Perform classification using one of approach A1, A2, A3, A4, or A5
    Using 1D/2D histograms and various ML models (XGBoost/CNN+XGBoost/RandomForest/K-Means/CatBoost/LightGBM)
    
    Parameters
    ----------
    approach: int
        The approach going to be used for classification
        - 1: 1D histograms with XGBoost
        - 2: 1D averaged histograms with XGBoost
        - 3: 2D histograms with CNN+XGBoost
        - 4: 2D averaged histograms with CNN+XGBoost
        - 5: 1D histograms with RandomForest
        - 6: K-Means clustering
        - 7: 1D histograms with CatBoost
        - 8: 1D histograms with LightGBM
    data: list of string
        Each string in the list contains the directory of one Dataset
    RR: numpy array
        1D array with experimental parameters "Ramp Rate" for each Dataset
    ...
    if approach == 1:
        ...
        # Approach A3 is 2D histograms with CNN+XGBoost model
        averageHist = False
        dimension = 2
    elif approach == 4:
        # Approach A4 is 2D averaged histograms with CNN+XGBoost model
        averageHist = True
        dimension = 2
    elif approach == 5:
        # Approach A5 is 1D histograms with RandomForest model
        averageHist = True
        dimension = 1
    elif approach == 6:
        # Approach A6 is K-Means clustering
        averageHist = True
        dimension = 1
    elif approach == 7:
        # Approach A7 is 1D histograms with CatBoost model
        averageHist = True
        dimension = 1
    elif approach == 8:
        # Approach A8 is 1D histograms with LightGBM model
        averageHist = True
        dimension = 1
    elif approach == 9:
        # Approach A9 is graph-based approach with GNN model
        averageHist = True
        dimension = 1
    else:
    ...
    conf_mat = np.zeros((num_group, num_group))
    for simIndex in range(100):
        ...
        if approach == 1 or approach == 2:
            # XGBoost model
            model = XGBClassifier(n_estimators=200, max_depth=2, verbosity=0)
        elif approach == 3 or approach == 4:
            # CNN + XGBoost model
            cnn_interLayer_model = cnn_pretrain(num_group, Train_Data, Train_Label, Test_Data, Test_Label)
            Train_Data = cnn_interLayer_model.predict(Train_Data, verbose=0)
            Test_Data = cnn_interLayer_model.predict(Test_Data, verbose=0)
            model = XGBClassifier(objective='multi:softmax', num_class=num_group, tree_method='hist', max_depth=2, verbosity=0, n_estimators=200)
        elif approach == 5:
            # Random Forest model
            ...
            # LightGBM model
            model = LGBMClassifier(n_estimators=300, max_depth=5, num_leaves=10)
        elif approach == 9:
            # GNN model
            Train_Data, Test_Data = gnn_pretrain(num_group, Train_Data, Train_Label, Test_Data, Test_Label)
            model = XGBClassifier(n_estimators=200, max_depth=2, verbosity=0)
        ...
        conf_mat = conf_mat + confusion_matrix(Test_Label, predictedLabels, labels=np.arange(num_group))
        if (simIndex + 1) % 10 == 0:
            print(f'    Finished iteration {simIndex + 1}')

    print(f"Data structure in run_classification: {type(data)}")
